...
set westPitch [expr {floor($westSpan / double($numPadsHeight - 1))}]
...
place_pad -row IO_WEST -location [expr {$westStart -  4*$westPitch}] "pad_status_o"        ; # pin no:  5 (TODO: requirement says UART RX)
place_pad -row IO_WEST -location [expr {$westStart -  5*$westPitch}] "pad_clk_i"           ; # pin no:  6 (TODO: requirement says UART TX)
...
set southPitch [expr {floor($southSpan / double($numPadsWidth - 1))}]
...
set eastPitch [expr {floor($eastSpan / double($numPadsHeight - 1))}]
...
set northPitch [expr {floor($northSpan / double($numPadsWidth - 1))}]
